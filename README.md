...
/plan          設計ドキュメント作成（人間と対話）
     ↓
/milestone     Milestone分割（.sefirot/milestones.json生成）
     ↓
/loop          自律ループ開始
...
`.sefirot/milestones.json` が生成される:

```json
{
  "source": "docs/tasks/20260310_1430_ユーザー認証設計/design.md",
  "milestones": [
    {
      "milestone": 1,
...
`/loop` 経由で実行すると、Planner/Builder/Verifier が判断に迷った場合に `.sefirot/milestones.json` の `questions` 配列に質問を書き込む。
