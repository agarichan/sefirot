...
```
/plan          設計ドキュメント作成（人間と対話）
     ↓
/gen-milestones   Milestone分割（milestones.json生成）
     ↓
/loop          自律ループ開始
...
```
/gen-milestones docs/tasks/20260310_1430_ユーザー認証設計.md
```

`milestones.json` が生成される:

```json
{
  "source": "docs/tasks/20260310_1430_ユーザー認証設計.md",
  "questions": [],
  "milestones": [
    {
      "milestone": 1,
...
`/loop` 経由で実行すると、Builder/Verifier が判断に迷った場合に `milestones.json` の `questions` 配列に質問を書き込む。
