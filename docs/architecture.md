...
構成要素は CLI（Python）と Claude Code スキルの2つ。外部サーバーやデーモンは不要。すべての状態は `.sefirot/milestones.json`（ランタイム状態、git 管理外）と設計ドキュメント（Markdown、git 管理）で管理される。
...
1. **Planner フェーズ** — タスクが未生成なら Planner を起動。設計ドキュメント（ライフサイクルディレクトリ内）とタスク一覧を生成し、`.sefirot/milestones.json` に書き込む。
...
| `/milestone` | 設計ドキュメントから `.sefirot/milestones.json` を生成 |
...
エージェント同士が直接通信することはない。すべての通信は `.sefirot/milestones.json` と設計ドキュメントを介して行われる:
...
3. **エージェント → ユーザー**: `.sefirot/milestones.json` の `questions` 配列に質問を書き込む。LoopEngine が検知して exit code 10 で停止し、スキルがユーザーに質問を提示する。

## 質問キュー

`--from-skill` モードで有効。Planner/Builder/Verifier が判断に迷った場合の流れ:

1. エージェントが `.sefirot/milestones.json` の `questions` 配列に質問を追加
...
project/
├── docs/tasks/
│   └── 20260310_1430_OAuth2認証設計/           # ライフサイクルディレクトリ
│       ├── design.md                           # 設計ドキュメント（/plan で生成）
│       ├── milestone-1.md                      # Planner が生成
│       └── milestone-2.md
├── .sefirot/
