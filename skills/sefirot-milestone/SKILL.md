...
>
> 例: `/milestone docs/tasks/20260310_1430_ユーザー認証設計/design.md`
...
5. Milestone を設計する
6. `.sefirot/milestones.json` を出力する（既存ファイルがあれば上書きする）
7. コミットは不要（`.sefirot/milestones.json` は git 管理外）

## 出力フォーマット

唯一の成果物は `.sefirot/milestones.json`。`.sefirot/` ディレクトリがなければ作成する。

```json
{
  "source": "docs/tasks/YYYYMMDD_HHMM_設計ドキュメント名/design.md",
  "milestones": [
    {
      "milestone": 1,
