...
2. `docs/tasks/{YYYYMMDD_HHMM}_milestone-__MILESTONE_NUMBER__.md` に設計ドキュメントを作成する
...
`milestones.json` の対象 Milestone に以下の 2 つを追加します（トップレベルは `{"source": "...", "milestones": [...]}` 形式）:
...
```
git add docs/tasks/*_milestone-__MILESTONE_NUMBER__.md milestones.json && git commit -m "chore: Milestone __MILESTONE_NUMBER__ plan"
```
