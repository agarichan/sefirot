...
Milestone __MILESTONE_NUMBER__ のフル設計ドキュメントを書き、タスク一覧を `milestones.json` に登録してください。

__CLAUDE_MD_SECTION__

## ステップ1: 状況把握

1. プロジェクトの設計ドキュメントやルールがあれば読む:
   - `docs/` 配下のアーキテクチャ、設計規約、品質ルール等
   - プロジェクト固有のルールファイル（上記 CLAUDE.md で指定されているもの）
2. アプリ仕様を把握する:
   - **元の設計ドキュメント `__SOURCE_DOC__` を必ず読む**（アプリ全体の仕様が書かれている）
   - 過去の Milestone の設計ドキュメントがあれば `__SOURCE_DIR__/` から探して読む
   - `milestones.json` を読む
3. 既存コードを Glob/Grep で調査し、現在の実装状況を把握する
4. git log --oneline -20 で直近の作業を確認する

### `[要ユーザー確認]` マーカーの処理

元の設計ドキュメントに `[要ユーザー確認]` マーカーが付いた項目がある場合、**必ず質問キューでユーザーに確認すること。** 自分で判断してはならない。

マーカーがない項目については、仕様が明確であれば自分で判断してよい。

## ステップ2: 設計ドキュメント作成

**Milestone __MILESTONE_NUMBER__ ゴール**: __MILESTONE_GOAL__

設計ドキュメントを `__SOURCE_DIR__/plan-m__MILESTONE_NUMBER__.md` に作成する。

### ドキュメント構成

```markdown
# Milestone __MILESTONE_NUMBER__: （ゴールの要約）
...
#### 作成するファイル
- `src/xxx/models/foo.py`
- `src/xxx/repositories/foo_repository.py`

#### 型定義・インターフェース
具体的な型シグネチャ、クラス定義、関数シグネチャを記述する。
プロジェクトの言語・フレームワークに合わせて書く。

#### import 先（既存コード）
- `src/xxx/shared/models/user` -> User
- `src/xxx/shared/result` -> Result
...
> **注意**: プロジェクトセットアップ（package.json, pyproject.toml, 設定ファイル等を触る作業）はコンフリクトが起きやすい。迷ったら直列（別 Wave）にする。
...
- **独立したタスクは必ず同一 Wave に配置する。** 依存関係がないのに別 Wave にすると直列実行になり、並列化の恩恵がゼロになる
- **trivial なセットアップ（ディレクトリ作成、空の `__init__.py` 配置等）は独立タスクにしない。** 実装タスクの前処理として統合すること。1 タスクあたり Builder セッション起動 + Verifier 検証で最低 $0.40 のオーバーヘッドがかかるため、数秒で終わる作業を独立タスクにするのは非効率
- **同じ Milestone の先行タスクが作ったファイルを参照する場合のみ** 別 Wave にする。「念のため直列」は禁止。コンフリクトが起きるか具体的に判断すること

## ステップ3: milestones.json 更新

`__MILESTONES_JSON_PATH__` の対象 Milestone に以下の 2 つを追加します（トップレベルは `{"source": "...", "milestones": [...]}` 形式）:
...
{
  "milestone": 1,
  "plan_doc": "__SOURCE_DIR__/plan-m1.md",
  "tasks": [
    {"id": "kebab-case-id", "description": "ドキュメント plan-mN のステップ A（YYY 機能）を実装する", "wave": 1, "done": false},
    {"id": "another-id", "description": "ドキュメント plan-mN のステップ B（ZZZ 機能）を実装する", "wave": 2, "done": false}
  ]
}
```
...
設計ドキュメントと milestones.json をコミットする。

```
git add __SOURCE_DIR__/plan-m__MILESTONE_NUMBER__.md __MILESTONES_JSON_PATH__ && git commit -m "chore: Milestone __MILESTONE_NUMBER__ plan"
```
