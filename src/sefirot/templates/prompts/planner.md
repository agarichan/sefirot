...
Milestone __MILESTONE_NUMBER__ のフル設計ドキュメントを書き、タスク一覧を `.sefirot/milestones.json` に登録してください。
...
設計ドキュメントを `__SOURCE_DIR__/milestone-__MILESTONE_NUMBER__.md` に作成する。
...
{
  "milestone": 1,
  "plan_doc": "__SOURCE_DIR__/milestone-1.md",
  "tasks": [
...
設計ドキュメントのみコミットする（milestones.json はランタイム状態のため git 管理外）。

```
git add __SOURCE_DIR__/milestone-__MILESTONE_NUMBER__.md && git commit -m "chore: Milestone __MILESTONE_NUMBER__ plan"
```
